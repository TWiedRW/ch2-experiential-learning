...
%DIF ADD manuscript-V3.tex   Mon May  6 19:05:41 2024
...
(\citeproc{ref-loy2021a}{2021}) \DIFdelbegin \DIFdel{have }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend demonstrated that student
...
possible and studies around the 1990s \DIFdelbegin \DIFdel{(\mbox{%DIFAUXCMD
...
skepticism of their use \DIFaddbegin \DIFadd{(\mbox{%DIFAUXCMD
\citeproc{ref-barfield1989}{Barfield and
Robless 1989}}\hskip0pt%DIFAUXCMD
; \mbox{%DIFAUXCMD
\citeproc{ref-fisher1997}{Fisher, Dempsey, and Marousky
1997}}\hskip0pt%DIFAUXCMD
)}\DIFaddend . Barfield and Robless (\citeproc{ref-barfield1989}{1989}) showed
...
(\citeproc{ref-kraus2020}{2020}) explored the 3D projections with the
...
discretion. \DIFaddbegin \DIFadd{Instructors for the June-July Summer 2023 courses did not
...
\DIFdelbegin \DIFdel{paragraph responses}\DIFdelend \DIFaddbegin \DIFadd{all modules, with the exception of the Post-Experiment reflection,
students are asked to comment on the experiment without an objectively
correct response.
}

\DIFadd{For paragraph responses in the Pre-Experiment and Abstract reflections}\DIFaddend ,
...
prompts. \DIFaddbegin \DIFadd{In the Post-Experiment reflection, the prompts have objectively
correct answers, but would require a subjective aggregation to indicate
the level of correctness. Instead, we opt to instead select at least one
student response that we consider to be ``most correct'' and a couple of
other responses that are either partially correct or entirely incorrect
to illustrate variability of the responses. }\DIFaddend The results of the graphics
...
\DIFdelbegin \section{\DIFdel{Figure Legends}}%DIFAUXCMD
...
asking about the goals of the research objectives\DIFaddbegin \DIFadd{, which were to to
evaluate the errors of ratio judgements between 2D and 3D graph types
...
included the comparisons of ratio judgements of the projections for 2D
...
but was mostly correct with their response of }\DIFaddend ``I think the purpose of
this experiment was for the researcher to gather data on how people
perceive, interpret, and understand 3D graphs.'' \DIFdelbegin \DIFdel{A third student correctly
...
would have included the comparisons of ratio judgements of the projections for 2D and 3D graphs. }\DIFdelend \DIFaddbegin \DIFadd{One student incorrectly
thought that we were testing differences in demographics by responding
``}{[}\DIFadd{t}{]}\DIFadd{hey could be trying to determine how different genders, ages,
etc. perceive the sizes of the bars in the graph. Demographics could
make a pretty significant difference.''
}\DIFaddend 

We then asked students ``\DIFdelbegin \DIFdel{What }\DIFdelend \DIFaddbegin {[}\DIFadd{w}{]}\DIFadd{hat }\DIFaddend hypotheses might the experimenters
have been testing?'' A correct response would include \DIFdelbegin \DIFdel{measuring differences in accuracy }\DIFdelend \DIFaddbegin \DIFadd{testing the
differences in errors }\DIFaddend of ratio judgements between \DIFdelbegin \DIFdel{2D and 3D graphs. One student
correctly identified this by
responding ``They }\DIFdelend \DIFaddbegin \DIFadd{graph types. With
\(\mu\) representing the error, the notation of this hypothesis, as
taught in STAT 218, is given by
}

\[\DIFadd{\text{H}_0:\mu_{\text{2D digital}}=\mu_{\text{3D digital (static)}}=\mu_{\text{3D digital (interactive)}}=\mu_{\text{3D printed}}}\]

\[\DIFadd{\text{H}_\text{A}: \text{At least one } \mu \text{ is different}}\]

\DIFadd{One example of a student providing a correct response was given by
stating ``}{[}\DIFadd{t}{]}\DIFadd{hey }\DIFaddend might have been testing if a 2D model is easier to
estimate its relative size to another when compared to a 3D model of
it.'' Other students replied with statements that would not be able to
be measured from the experiment, with one student responding ``How
taking Statistics 218 effects how you can compare two groups'' and
another student saying ``That 2d is preferred over 3d. It cleans up the
data presentation.''
...
with their responses and that they were on the path of statistical
thinking.

\DIFaddbegin \DIFadd{Having students participate in experiments is not uncommon, possibly
attributed to the readily available convenient sample within academia
and potential applications to introduce new course material
(\mbox{%DIFAUXCMD
\citeproc{ref-margaret1994}{Margaret 1994}}\hskip0pt%DIFAUXCMD
). However, these studies
typically end for students after completing the experiment
(\mbox{%DIFAUXCMD
\citeproc{ref-mcgowan2011}{McGowan 2011}}\hskip0pt%DIFAUXCMD
;
\mbox{%DIFAUXCMD
\citeproc{ref-zacks1998}{Zacks et al. 1998}}\hskip0pt%DIFAUXCMD
;
\mbox{%DIFAUXCMD
\citeproc{ref-fisher1997}{Fisher, Dempsey, and Marousky 1997}}\hskip0pt%DIFAUXCMD
). In our
study, we integrated the graphics experiment as a course project with
the addition of reviewing research material.
}

\DIFaddend What we found was that the graphics project found success within the
recommendations of the GAISE College Report \DIFaddbegin \DIFadd{by using real data to
illustrate the contextual purpose of scientific research
}\DIFaddend (\citeproc{ref-carver}{Carver, College, and Everson 2016}). Students
...
participated provided meaningful answers that displayed \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{some }\DIFaddend level of
...
\DIFaddendFL \toprule
Semester & Number of \DIFdelbeginFL \DIFdelFL{Sections }%DIFDELCMD < & %%%
\DIFdelFL{Number of }\DIFdelendFL Students\\
\midrule
Summer 2023 (May-June) & \DIFdelbeginFL \DIFdelFL{1 }%DIFDELCMD < & %%%
\DIFdelendFL 17\\
Summer 2023 (July-Aug) & \DIFdelbeginFL \DIFdelFL{1 }%DIFDELCMD < & %%%
\DIFdelendFL 23\\
Fall 2023 (May-June) \DIFdelbeginFL %DIFDELCMD < & %%%
